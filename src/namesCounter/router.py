...
async def get_current_name_counter(
    token: str = Header(), session: AsyncSession = Depends(get_async_session)
):
...
async def get_names_counters(
    ...
    names_counter=Depends(get_current_name_counter),
    limit: int = 10,
    offset: int = 0,
):
    """
...
@names_counter_router.post("/login", response_model=LoginRead)
